...
public class PoConnectionManager {
	...
	public boolean disconnect() {
		...
		try {
			...
			System.out.println(TAG+ "IOException while closing socket.");
		}
		...
		try {
			...
			System.out.println(TAG+ "Interrupted while join()'ing read/write threads.");
			return false;
		}
		//OutMsgQ.clear();
		System.out.println(TAG+ "Both read/write threads Join()'ed.");
		System.out.println(TAG+ "disconnect() was successful.");
		isConnected = false;
		return true;
	}
	...
	public boolean connect() {
		...
		try {
			...
			if(sockch.isConnected()) {
				isConnected = true;
				System.out.println(TAG+ " Socket is connected.");
			}
			else {
				System.out.println(TAG+ " Called connect() but socket neither connected nor threw exception.");
				return false;
			}
		} catch (IOException e) {
			System.out.println(TAG+ "Failed to connect to the directory: "+e.toString());
			isConnected = false;
			return false;
		}
		...
		if(!writer_thread.isRunning()) {
			try {
				...
				System.out.println(TAG+ " writer thread caught exc: "+e.toString());
			}
		}
	...
	public void startUpdateThread() {
		System.out.println(TAG+ "Starting update thread.");
		killUpdateThread();
