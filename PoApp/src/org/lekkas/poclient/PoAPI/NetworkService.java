...
public final class NetworkService implements NetworkServiceI {	
	...
	public void JNICallback_SendPacket(byte[] payload, byte len, char source, char dest) {
		System.out.println(TAG+ "SendPacket CB: payload.length = "+payload.length);
		System.out.println(TAG+ "SendPacket CB: len = "+(int)len);
		...
		// System.out.println(TAG+ "Src: buf[0]="+buf[0]+", buf[1]="+buf[1]);
		// System.out.println(TAG+ "Dst: buf[2]="+buf[2]+", buf[3]="+buf[3]);
		System.out.println(TAG+ "Src: "+byteArrayToAscii(buf, 0, 1));
		System.out.println(TAG+ "Dst: "+byteArrayToAscii(buf, 2, 3));
	...
	public char JNICallback_JoinNetwork() {
		...
		System.out.println(TAG+ "Returning local myAddr: "+(int)c);
		return c;
	}
