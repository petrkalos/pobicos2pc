...
public class EventHandlerThread extends Thread{
	...
	public void run() {
		System.out.println(TAG+ "Event Handler Thread started!");
		Running = true;
		...
		System.out.println(TAG+ "MiddlewareManager class loaded successfuly.");
		
		mgr = MiddlewareManager.getInstance();
		if (this.getClass().getClassLoader() == mgr.getClass().getClassLoader() )
			System.out.println(TAG+ "Classloader of evt handler thread is the same with MiddlewareManager");
		...
		while(!Thread.currentThread().isInterrupted()) {
			try {
				...
				System.out.println(TAG+ "Event Handler Thread Interrupted.Queue Exception"+e.toString());
				cleanup();
				return;
			}	
		} 
		System.out.println(TAG+ "Event Handler Thread Interrupted.");
		cleanup();
	}
	...
	public void DispatchEvt(PoEvent evt) {
		...
		if(evt instanceof PoTimerFired) {
			...
			System.out.println(TAG+"Unknown event.");
