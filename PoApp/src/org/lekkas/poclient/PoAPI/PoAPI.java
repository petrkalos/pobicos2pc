...
public final class PoAPI implements PoAPIInterface {
	...
	private static STATE status = STATE.STOPPED; 
	...
	public void start() {
		...
		if (PoAPI.class.getClassLoader() == handler_thread.getClass().getClassLoader())
			System.out.println(TAG+ "Classloader of POAPI is the same with evt handler thread");
	...
	public void stop() {
		...
		try {
			handler_thread.join();
			System.out.println(TAG+ "Main thread join(): Event handler thread is killed");
			status = STATE.STOPPED;
			handler_thread = null;
			EventQueue.clear();
		} catch(InterruptedException e) {
			System.out.println(TAG+ "PoAPI interrupted while join()'ing evt handler thread.");
		}
	}
