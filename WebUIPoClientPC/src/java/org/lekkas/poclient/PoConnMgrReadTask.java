...
import org.kalos.clientmgr.Network_Msg;
                ...
                while (it.hasNext()) {
                    ...
                    if (flag == Network_Msg.POBICOS_MSG) {
                        ...
                        if (isServer(srcaddr)) {
                            ...
                            if (n == null) {
                                double lat = payload.getDouble(Network_Msg.LAT_OFFSET);
                                double lon = payload.getDouble(Network_Msg.LON_OFFSET);
                                Log.w(TAG, "Add new node lat:"+lat+" lon:"+lon);
                                System.out.println("1PETROS Add new node lat:"+lat+" lon:"+lon);
                                char addr = payload.getChar(0);
                                n = new NodeInfo(sockch, addr,lat,lon);
