		...
		while(!Thread.currentThread().isInterrupted()) {
			try {
				...
				if(msg.msg_type == Network_Msg.POBICOS_MSG){
					...
					buf.position(0);
					if(isServer(destaddr)){
						Log.w(TAG, "PETROS send message to server "+(int)destaddr);
						SocketChannel servsock = ServerManager.getInstance().getServerSocket(destaddr);
						while(buf.hasRemaining()) servsock.write(buf);
						Log.w(TAG, "PETROS message successfull sended "+(int)destaddr);
					}else{
						while(buf.hasRemaining()) sockch.write(buf);
					}
				}else{
