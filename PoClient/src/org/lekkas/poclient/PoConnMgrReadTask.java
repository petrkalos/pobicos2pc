...
import org.kalos.server.ServerManager;
import org.kalos.server.ServerNode;
...
public class PoConnMgrReadTask extends Thread implements Runnable {
	...
	public void run() {
		...
		network_msg_header = ByteBuffer.allocate(Network_Msg.HEADER_LEN); // Allocate
																			// space
																			// for
																			// Msg_type(1),
																			// Seq(1)
		...
		while (!Thread.currentThread().isInterrupted()) {
			try {
				 ...
				 * MSG Reply format:
				 * 
				 * Msg Type: byte: 1 byte Len: byte: 1 byte Payload: byte[]:
				 * {Len} bytes
				 * 
				 * Where Payload:
				 * 
				 * 1) For POBICOS messages: Src addr : char : 2 bytes Dest addr
				 * : char : 2 bytes Msg Len : byte : 1 byte Msg : byte[]: {Msg
				 * Len} bytes;
				 * 
				 * 2) For registration messages: a)2 bytes, (Registered node
				 * address) + 4 bytes(seed) for welcome messages b)no payload
				 * for pong messages
				 */
				Log.w(TAG, "Read()'ing");
				/*
				...
				while (it.hasNext()) {
					...
					Log.w(TAG, "read() Header: " + ret
							+ " bytes. Length of payload = " + len);
					if (len != 0) {
					...
					if (flag == Network_Msg.REG_REPLY_WELCOME) {
						Log.w(TAG, "GOT REG_REPLY_WELCOME! addr:"
								+ byteArrayToAscii(payload.array()));
						byte[] addr = { payload.get(0), payload.get(1) };
						byte[] seed = { payload.get(2), payload.get(3),
								payload.get(4), payload.get(5) };
						...
						PoRegistryService.getInstance().setState(
								PoRegistryService.STATE.REGISTERED);
						sendRegistryIntent(); // first time connection; start
												// registry update thread.
					}
					if (flag == Network_Msg.REG_REPLY_FAIL) {
						Log.w(TAG, "Registration failed!");
						PoRegistryService.getInstance().setState(
								PoRegistryService.STATE.REJECTED);
					...
					if (flag == Network_Msg.REG_REPLY_PONG) {
						...
						PoRegistryService.getInstance().setState(
								PoRegistryService.STATE.REGISTERED);
						if (!PoConnectionManager.getInstance()
								.isRegistryUpdateThreadRunning())
							sendRegistryIntent(); // needed to restart update
													// thread in case we have
													// just reconnected.
					}
					if (flag == Network_Msg.POBICOS_MSG) {
						...
						PoAPI.getEventQueue().offer(
								new PoNetworkPacketRx(flag, (byte) len, payload
										.array()));
					}
					...
					if (flag == Network_Msg.SERVER_SEARCH_RES) {
						...
						Log.w(TAG, "PETROS: server ip for " + (int) poaddr
								+ " is " + ip + ":" + port);
						ServerNode node = new ServerNode(poaddr, ip, port);
	...
	private void sendRegistryIntent() {
		Intent myIntent = new Intent(PoApp.getMiddlewareService(),
				PoConnStatusBR.class);
		myIntent.putExtra("registry_event", "");
		PoApp.getMiddlewareService().getApplicationContext()
