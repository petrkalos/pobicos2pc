...
public class RequestHandler implements Runnable {
    ...
    private void process() throws IOException {
        ...
        if (flag == Network_Msg.POBICOS_MSG) {
            ...
            if (v) {
                System.out.println(TAG + "Source address: " + byteArrayToAscii(out_msg.array(), 2, 3));
            }
            if (v) {
                System.out.println(TAG + "Destination address: " + byteArrayToAscii(out_msg.array(), 4, 5));
            }
    ...
    private void write() throws IOException {
        ...
        if (op == OP.SEND_REG_MSG) {
            if (v) {
                System.out.println(TAG + "Sending packet: " + byteArrayToAscii(out_msg.array()));
            }
            while (out_msg.hasRemaining()) {
                sock.write(out_msg);
            }
            if (v) {
                System.out.println(TAG + "Sent REG reply. Wrote " + out_msg.capacity() + " bytes.");
            }
        }
        ...
        if (op == OP.SEND_PO_MSG) {
            ...
            if (dstId == (char) 0xffff) {
                if (v) {
                    System.out.println(TAG + "Node " + byteArrayToAscii(out_msg.array(), 2, 3) + " is broadcasting.");
                }
